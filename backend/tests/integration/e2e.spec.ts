...
describe('Testing end to end flow', () => {
  ...
  describe('Testing list items retrieval', () => {
    ...
    it('endpoint should return item that was created previously and have all expected fields', async () => {
      const createdItem: { id: string; name: string; status: string; createdBy: string; listId: string; createdAtTimestamp: string } =
        requests.getListItems.output.find(
          (item: { id: string; name: string; status: string; createdBy: string; listId: string; createdAtTimestamp: string }) =>
      ...
      expect(createdItem?.createdBy).to.exist.and.be.a('string');
